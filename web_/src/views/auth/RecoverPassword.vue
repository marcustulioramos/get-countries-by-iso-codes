      ...
      <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 mx-auto">
        <div class="mb-4">
          ...
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            id="username" type="text" v-model="user.email" placeholder="Email">
        </div>
        <div class="flex items-center justify-between">
          <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
            type="button" @click="proceedLogin">
            Reset Password 
          </button>
  ...
  export default {
    ...
    methods: {
      proceedLogin() {
        ...
        axios.post(`${this.apiUrl}/forgot-password`, this.user).then((response) => {
          console.log(response)
          if (response.status === 200) {
            this.$toasted.show('Email was sent!', { type: "info" })
            // setInterval(() => { this.$router.push('app') }, 3000);
            setTimeout(() => {
